...
/// Regression for a bug where a large field never wrote a career record at
/// all. `_finishHeadless` used to shortcut straight to `_settleByChips`
/// (ranking the field by chip count instead of actually playing it out)
/// whenever more than 72 players remained when the human busted, and
/// `_settleByChips` called `state.declareChampion()` directly, bypassing
/// `_maybeFinish()` and the `_recordCareer()` call inside it. A small field
/// always went through the hand-by-hand `step()` loop, which does call
/// `_maybeFinish()`, so this only ever showed up on a big field — exactly
/// what "play a tournament with every personality" produces.
///
/// `_finishHeadless` no longer shortcuts by field size at all — every bust
/// plays the rest of the field out for real — so `_settleByChips` shouldn't
/// run here either, but `_recordCareer()` still needs to fire regardless of
/// which path finishes the tournament, hence this stays a real regression
/// guard rather than dead weight.
void main() {
  test(
    'a large field the human busts out of still records a career result',
    () async {
